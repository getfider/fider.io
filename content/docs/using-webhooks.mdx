...
If you never used a Webhook before, we recommand that you read <a href="#webhook-configuration">this section about Webhook configuration</a>.
...
The power of this feature reside in its customization. Each Webhook has a name to quickly recognize it, a type defining its trigger event, a status
...
The status define whether the Webhook could be triggered by events of its type or not. We highly recommend you to [test](#webhook-testing) your Webhook before
...
The content is the main element in your Webhook. You define here the data sent as HTTP request body. Most of time, this payload will include information
about the event that just happened. Like in the URL, you can use [Go template formatting](#writing-go-templates) here. Do not hesitate to check the integrated
...
This is the [HTTP request method](https://developer.mozilla.org/en-US/docs/Web/HTTP/Methods) of the request triggered by the Webhook. Most of time, when sending
data in the **Content**, it should be `POST`. If you don't send any data, you should use `GET`. Check your targeted application's documentation to be sure.
...
native `text/template` Go package. This is a powerful engine, but creating a basic templates is not so complicated. You can write text as you want and insert a
...
*(Read more about time formatting [here][go-time-format])*
...
You need to properly encode values like that when using them in JSON fields for example. Indeed, with the following **bad** configuration:
```gotemplate
{
...
Please avoid these trouble and use the safety functions like `quote`, `urlquery`, `stripHtml`...
...
---|---
`author_avatar` | http://localhost:3000/static/avatars/letter/1/Test
`author_email` | test@test.fr
`author_id` | 1
`author_name` | Test
`author_role` | administrator
`comment` | An example \*\*comment\*\* on a post.
`post_author_avatar` | https://getfider.com/images/logo-100x100.png
...
`post_response` | true
`post_response_author_avatar` | http://localhost:3000/static/avatars/letter/1/Test
...
`post_url` | http://localhost:3000/posts/36/example-dummy-post-title
`post_votes` | 7
`tenant_id` | 1
`tenant_invitation` | *&lt;empty&gt;*
`tenant_locale` | fr
`tenant_logo` | https://getfider.com/images/logo-100x100.png
`tenant_name` | Testing
`tenant_status` | active
`tenant_subdomain` | default
`tenant_url` | http://localhost:3000
`tenant_welcome_message` | *&lt;empty&gt;*
...
Don't forget the **Preview** field to help you experiment with these functions!
